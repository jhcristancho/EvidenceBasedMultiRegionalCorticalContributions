...
for q=1:size(subjects,1)
    clearvars -except q subjects Fs maxStates lp_freq fac_resample Type C lenC numIt modelT;
    subject=subjects(q,:);
...
%         stim_test = cell2mat(stim_test);
        ...
        lolo = 1:nRounds2;
        myAcc = zeros(lenC,nRounds2);
        for elRound = 1:nRounds2
            dataTrain_HG_train = dataTrial_HG_train(lolo~=elRound);
            dataTrain_HG_train = cell2mat(dataTrain_HG_train);
            stimTrain_train = stim_train(lolo~=elRound);
            stimTrain_train = cell2mat(stimTrain_train);
            dataTrain_HG_test = dataTrial_HG_train(lolo==elRound);
            dataTrain_HG_test = cell2mat(dataTrain_HG_test);
            stimTrain_test = stim_train(lolo==elRound);
            stimTrain_test = cell2mat(stimTrain_test);
            
            for elC = 1:lenC                
                C_val = C(elC);
%                 disp(['      ... fold ' num2str(elRound) ' and C ' num2str(C_val) '...']);
                opts = ['-q -c ' num2str(C_val) ' -t 0 -b 1 -m 2048'];
                opts_test = '-q -b 1';
                model = libsvmtrain(double(stimTrain_train),dataTrain_HG_train,opts);
                maxprobSVM = libsvmpredict(double(stimTrain_test),dataTrain_HG_test,model,opts_test);
                isEqualMaxlogStim = (maxprobSVM(:) == stimTrain_test(:));
                myAcc(elC,elRound) = mean(isEqualMaxlogStim);
                
            end
        end
        
        accMean = mean(myAcc,2);
        [~,indC] = max(accMean);
        C_val = C(indC);   
...
%         stim_test = stim_cell(vecRounds==nTrain);
        ...
        opts = ['-q -c ' num2str(C_val) ' -t 0 -b 1 -m 2048'];
        opts_test = '-q -b 1';
        model = libsvmtrain(double(stim_train),dataTrial_HG_train,opts);
        maxprobSVM = libsvmpredict(double(stim_test),dataTrial_HG_test,model,opts_test);
        isEqualMaxlogStim = (maxprobSVM(:) == stim_test(:));
        
        avgAcc(nTrain, 1) = mean(isEqualMaxlogStim);
        avgAcc(nTrain, 2) = C_val;
        fufu = unique(stim_test);
        testCP{nTrain} = classperf(stim_test,maxprobSVM,'Positive',fufu(2:end),'Negative',fufu(1));
        toc;
    end
    save(['data' filesep subject filesep subject '_LSVM_SMreg_lpfreq_' num2str(lp_freq(lafreq)) '_V2_5.mat'],'myAcc','avgAcc','dataTrial_HG_cell','stim_cell','testCP');
end