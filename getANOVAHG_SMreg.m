...
cell_bacc = cell(numType*numModels*numSubjects,1);
...
for q=1:numSubjects
    ...
    for elmodelo = 1:numModels
        ...
        if ~isempty(strfind(modelT{elmodelo},'crf'))
            nomfil = ['data' filesep subject filesep subject '_' Type{1} '_model_' upper(modelT{elmodelo}) '_SMreg_numIt' num2str(numIt) '_lpfreq_' num2str(lp_freq) '_V2_5.mat'];
            
        else
            nomfil = ['data' filesep subject filesep subject '_' upper(modelT{elmodelo}) '_SMreg_lpfreq_' num2str(lp_freq) '_V2_5.mat'];
        end
...
cell_bacc = vertcat( cell_bacc{:} );

cell_group = {cell_mode,cell_subs};
cell_varnames = {'Classifier','Subject'};
% return
...
[anova_p_bacc,anova_table_bacc,anova_stats_bacc,anova_terms_bacc] = anovan(cell_bacc,cell_group,'model','linear','varnames',cell_varnames,'display','off');
% figure;
[anova_c1_bacc,anova_m1_bacc,anova_h1_bacc,anova_gnames1_bacc] = multcompare(anova_stats_kappa,'dimension',1,'ctype','hsd','display','off');
% figure;
[anova_c2_bacc,anova_m2_bacc,anova_h2_bacc,anova_gnames2_bacc] = multcompare(anova_stats_kappa,'dimension',2,'ctype','hsd','display','off');

save(['data' filesep 'compare_all_Algorithm_V2_5.mat'],'-regexp','^cell_','^anova_','^avgAcc','^mat');