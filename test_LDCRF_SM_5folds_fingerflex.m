...
lp_freq = [8];
% lp_freq = [100];
% fac_resample = [100]; %Resampling of 10 Hz
...
for q=1:size(subjects,1)
    clearvars -except ver_fil q subjects Fs maxStates lp_freq fac_resample Type winMax delayMax numIt modelT;
    subject=subjects(q,:);
    ...
    for eltipo = 1:numel(Type)
        ...
        for nTrain = 1:numRounds
            ...
            testCP{nTrain} = classperf(stim_test(:),maxlogCRF(:),'Positive',fufu(2:end),'Negative',fufu(1));
            toc;
            save(['data' filesep subject filesep subject '_' Type{eltipo} '_model_' upper(modelT) '_SMreg_numIt' num2str(numIt(elC)) '_lpfreq_' num2str(lp_freq(lafreq)) '_tmpRound_' num2str(nTrain) '.mat'],'tmpAcc','dataTrial_HG_train','dataTrial_HG_test','stim_train','stim_test','winMax','maxStates','myAcc','testCP','avgAcc','statsCRF');
            
        end
        
        save(['data' filesep subject filesep subject '_' Type{eltipo} '_model_' upper(modelT) '_SMreg_numIt' num2str(numIt(elC)) '_lpfreq_' num2str(lp_freq(lafreq)) '_V' ver_fil '.mat'],'myAcc','testCP','avgAcc','dataTrial_HG_cell','stim_cell');
        for nTrain = 1:numRounds
