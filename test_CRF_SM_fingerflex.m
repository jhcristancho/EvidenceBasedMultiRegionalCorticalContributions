...
%     dataTrial_HG = dataTrial_HG(:,colMaxR2);
    ...
    dataTrial_HG = downsample(dataTrial_HG,round(Fs/100));
    stim = downsample(stim,round(Fs/100));
    [numData,numCol] = size(dataTrial_HG);
    
    numData_10 = ceil(numData / 10);
    
    dataTrial_HG = cat(1,dataTrial_HG,zeros(10*numData_10-numData,numCol));
    stim = cat(1,stim(:),zeros(10*numData_10-numData,1));    
    dataTrial_HG = dataTrial_HG';
    stim = stim';
    
    disp('Partitioning...');
    dataTrial_HG_seq = reshape(dataTrial_HG,[numCol,numData_10,10]);    
    dataTrial_HG_cell = mat2cell(dataTrial_HG_seq,numCol,numData_10,ones(1,10));    
...
%     C = 0.05:0.05:1;
    ...
    for elC = 1:lenC
        ...
        paramHCRF = struct('modelType','crf','caption','CRF','optimizer','lbfgs','regFactor',C,'windowSize',0,'rangeWeigths',[-1 1],'debugLevel',0);
...
%         paramHCRF.nbHiddenStates = maxStates;
    ...
    save(['data' filesep subject filesep subject '_res_CRF_SM_windowSize_0.mat'],'myAcc','C','dataTrial_HG_cell','stim_cell','numIt');
end